...
def generate_html_report(report, path):
    ...
    for finding in report.get("findings", [])[:100]:
        rows.append("<tr>" f"<td>{html.escape(str(finding.get('severity', '')))}</td>" f"<td>{html.escape(str(finding.get('title', '')))}</td>" f"<td>{html.escape(str(finding.get('host', '')))}</td>" f"<td>{html.escape(str(finding.get('port', '')))}</td>" "</tr>")
    summary = report["summary"]
    drift = report.get("drift", {}).get("summary", {})
    drift_html = ""
    if drift:
        drift_html = ("<h2>Security Drift</h2>" f"<p>New findings: <b>{int(drift.get('new_findings', 0))}</b> &nbsp; " f"New hosts: <b>{int(drift.get('new_hosts', 0))}</b> &nbsp; " f"Hosts with port changes: <b>{int(drift.get('hosts_with_port_changes', 0))}</b> &nbsp; " f"Risk delta: <b>{float(drift.get('risk_score_delta', 0)):+.1f}</b></p>")
...
def main():
    ...
    if args.purple_team:
        ...
        print(json.dumps([simulator.simulate_attack_chain(t, args.chain) for t in targets], indent=2)); return
    if args.zero_trust:
    ...
    if args.deception:
        ...
        out = deception.deploy_honeypots(args.environment) + deception.deploy_honeytokens(args.environment) if args.deception == "deploy" else deception.check_alerts() if args.deception == "check" else deception.cleanup()
    ...
    if args.baseline: report["drift"] = compare_reports(load_report(args.baseline), report)
    if args.inventory_output: _write_json(report["asset_inventory"], args.inventory_output)
    if args.format == "json": save_report(report, args.output)
    else: generate_html_report(report, args.output)
    if args.save_baseline: save_report(report, args.save_baseline)
    print(f"Scan complete: {args.output} | findings={report['summary']['total_findings']} | risk={report['summary']['risk_score']:.1f}/100 | assets={report['summary']['host_count']}")
    if report.get("drift"):
        drift = report["drift"]["summary"]
        print(f"Drift: new_findings={drift['new_findings']} | new_hosts={drift['new_hosts']} | port_changes={drift['hosts_with_port_changes']} | risk_delta={drift['risk_score_delta']:+.1f}")
        if args.fail_on_regression and drift["security_regression"]: raise SystemExit(2)
