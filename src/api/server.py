...
from src.core.support import support_info

app = FastAPI(title="Hobbit-NG API", version="3.2.0")
security = HTTPBearer()
...
def verify_token(credentials: HTTPAuthorizationCredentials = Security(security)):
    ...
    tenant = TENANTS.get(user.get("tenant_id")) if user else None
...
async def login(request: LoginRequest):
    if os.getenv("HOBBIT_DEV_AUTH") != "1":
        raise HTTPException(status_code=503, detail="Development login disabled; configure an identity provider")
    expected = os.getenv("HOBBIT_DEV_ADMIN_PASSWORD", "")
    if not expected or request.username != "admin-1" or request.password != expected:
...
@app.get("/support")
async def support():
    """Public optional-support metadata. Donation status never gates features."""
    return support_info()
...
async def create_scan(request: ScanRequest, auth: Dict = Depends(verify_token)):
    if not request.authorized:
        raise HTTPException(status_code=400, detail="authorized=true is required and confirms permission to assess the supplied targets")
    tenant_id = auth["user"]["tenant_id"]
    scan_id = hashlib.sha256(f"{tenant_id}:{_now().isoformat()}".encode()).hexdigest()[:16]
    count = int(await redis_client.get(f"rate_limit:{tenant_id}") or 0)
    if count >= auth["tenant"]["rate_limit"]:
        raise HTTPException(status_code=429, detail="Rate limit exceeded")
    pipe = redis_client.pipeline()
    pipe.incr(f"rate_limit:{tenant_id}")
    pipe.expire(f"rate_limit:{tenant_id}", 3600)
    await pipe.execute()
    task = celery_app.send_task("src.workers.scanner.run_scan", args=[scan_id, tenant_id, request.model_dump()], queue="hobbit-scans")
    await redis_client.setex(
        f"scan:{scan_id}",
        86400,
        json.dumps({"status": "queued", "tenant_id": tenant_id, "created_at": _now().isoformat()}),
    )
...
async def status(scan_id: str, auth: Dict = Depends(verify_token)):
    ...
    scan = json.loads(raw)
    if scan.get("tenant_id") != auth["user"]["tenant_id"]:
...
async def results(scan_id: str, auth: Dict = Depends(verify_token)):
    ...
    if scan.get("status") != "completed":
        raise HTTPException(status_code=409, detail=f"Scan is {scan.get('status', 'unknown')}")
...
async def remediate(request: RemediationRequest, auth: Dict = Depends(verify_token)):
    if not request.dry_run:
        raise HTTPException(status_code=403, detail="Automatic remediation execution is disabled in the baseline build")
    return {
        "remediation_id": "rem-" + hashlib.sha256(str(_now()).encode()).hexdigest()[:8],
        "dry_run": True,
        "actions": [],
    }
...
async def ws(websocket: WebSocket, scan_id: str):
    ...
    await pubsub.subscribe(f"scan_updates:{scan_id}")
    try:
        async for message in pubsub.listen():
            if message.get("type") == "message":
                await websocket.send_text(message["data"])
    finally:
        await pubsub.unsubscribe(f"scan_updates:{scan_id}")
        await pubsub.close()
...
async def health():
    ...
    return {
        ...
        "version": "3.2.0",
        "components": {"redis": redis_ok, "worker": "external"},
    }
